...
test_priority_fifo (void) 
{
  ...
  ASSERT (thread_get_priority () == PRTY_DEFAULT);
  ...
  thread_set_priority (PRTY_DEFAULT + 2);
  for (i = 0; i < THREAD_CNT; i++) 
    {
      ...
      d->lock = &lock;
      d->op = &op;
      thread_create (name, PRTY_DEFAULT + 1, simple_thread_func, d);
    }

  thread_set_priority (PRTY_DEFAULT);
