...
bool timer_sleep_compare(const struct list_elem *a,
...
bool tick_comparator(const struct list_elem *a,
                      const struct list_elem *b,
                      void *aux UNUSED){

  struct thread *x = list_entry(a, struct thread, elem);
  struct thread *y = list_entry(b, struct thread, elem);
  if(x->wakeup_t < y->wakeup_t)
    return true;
  return false;
}


/* change thread status and reschedule
 */
void thread_sleep(void)
{
  ...
  list_insert_ordered(&sleep_list, &t->elem, tick_comparator, NULL);
  schedule();
}
